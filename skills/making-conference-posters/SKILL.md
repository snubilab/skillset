...
5. **Audit.** Fix everything it reports, repeat until clean. Add `--conformance` to also fail on any font size or spacing length that is not one the template declares — this is the check for the first-priority rule, and nothing else enforces it. Expect it to be noisy on a poster not built from this template; on one that was, it should be silent. A `--root` or `--card` selector matching nothing is an error, not a pass. Exit 0 alone is not clean: `DEAD SPACE` and `LETTERBOX` are report-only and never fail the exit code — read the output, do not gate on the status.
...
The audit proves nothing is *clipped* **by the measurements it takes**, and it cannot read. Things it does not see, so do not treat a clean run as proof of them: **per-element horizontal overflow** — width is checked at the root only; **overflow into a box's own padding** — an element can sit a padding-height plus `--tol` past its content box and still exit 0; and **per-glyph font fallback** — the font check is family-level, so a codepoint missing from a subset silently substitutes another face. Ink lost to a `clip-path` corner **is** now reported (`INK cut off by clip-path`), which is how a heading too long for its banner surfaces — it used to print nothing at all. What ships past a clean audit is wrong content, so read the proof for each of these:
...
| Trimming an asset's whitespace with a fixed inset before finding its ink | A glyph, an axis label or a logo's descenders are shaved off; the poster audits clean forever because the loss is inside the image file, not in the layout | An inset that strips a border is destructive on an asset already cropped tight. Compute the ink box on the **untrimmed** image, and if you must strip a border, verify one exists first. Then check the edge rows still carry ink: `python3 -c "from PIL import Image; import numpy as np; a=np.asarray(Image.open('x.png').convert('L')); print((a<200).sum(axis=1)[:3], (a<200).sum(axis=1)[-3:])"` — a non-zero count on the outermost row means you cut into the mark. |
| Judging a suspected crop from the exported page | You verify the PDF, the PNG and the PPTX geometry, find the element fully inside its box, and report no defect — while the thing the reader sees cut is real | Those checks answer "is the element clipped by the layout". They cannot answer "is the artwork inside the element already missing its bottom". When someone says an asset looks cut and the layout is clean, stop checking the page and open the **asset file**. |
...
| A logo copied out of a browser | Prints soft at sheet size; fine hairlines and small caps mush | A right-click copy gives you the *displayed* size, often ~500 px. **Measure the ink, not the file** — logo files are mostly padding, and one that looks like the biggest of your candidates can hold the smallest mark: in one real set a 380x170 file trimmed to 150x65 of actual logo while a 300x132 file held 294x126. `audit_fit.py` reports effective dpi per image and fails below `--min-dpi` (default 150). Get the vector/original, or shrink the placement until it clears the floor. Never upscale — it invents no detail and smears edges. |
